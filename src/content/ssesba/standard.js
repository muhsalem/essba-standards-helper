  ...
  function initSectorSearch() {
    ...
    var tiers = Array.prototype.map.call(root.querySelectorAll('.tier'), function (tier, i) {
      ...
      var cards = Array.prototype.map.call(tier.querySelectorAll('.sec'), function (card) {
        ...
        Array.prototype.forEach.call(card.querySelectorAll('[data-en]'), function (el) { parts.push(el.getAttribute('data-en') || ''); });
        return { el: card, hay: parts.join(' ').toLowerCase() };
      });
      return { el: tier, cards: cards };
    });
    var total = tiers.reduce(function (n, t) { return n + t.cards.length; }, 0);
    function apply() {
      ...
      if (count) {
        count.textContent = (q || ti)
          ? (en ? shown + ' of ' + total + ' sectors' : 'عرض ' + shown + ' من ' + total + ' قطاعًا')
          : (en ? total + ' sectors' : total + ' قطاعًا');
      }
    }
    input.addEventListener('input', apply);
    tierSel.addEventListener('change', apply);
    if (clear) clear.addEventListener('click', function () { input.value = ''; tierSel.value = ''; apply(); input.focus(); });
    apply();
  }
