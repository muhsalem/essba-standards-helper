...
export function StandardContentPage({ lang }: { lang: Lang }) {
  ...
  return (
    <SiteShell
      ...
      title={
        en
          ? "The reference standard for assessing economic activities"
          : "المعيار المرجعي لتقييم وتصنيف الأنشطة الاقتصادية"
      }
    >
          ...
          <div className="flex flex-wrap gap-3">
            <Button asChild className="bg-brand-navy text-primary-foreground">
              <Link to={en ? "/en/request" : "/request"}>
                <ClipboardCheck />
                {en ? "Request assessment" : "اطلب تقييمًا"}
              </Link>
            </Button>
            <Button asChild variant="outline">
              <Link to={en ? "/en/assessment" : "/assessment"}>
                <Scale />
                {en ? "Try the model" : "جرّب النموذج"}
              </Link>
            </Button>
          </div>
        </div>
          ...
          <ol className="space-y-1 border-s border-brand-gold/40 ps-4 text-sm">
            {sections.map((s, i) => (
              <li key={s.id}>
                <a
                  href={`#section-${s.id}`}
                  className="flex gap-2 rounded px-1 py-1.5 leading-6 text-muted-foreground transition-colors hover:text-brand-navy"
                >
