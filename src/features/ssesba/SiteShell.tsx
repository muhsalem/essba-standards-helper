...
import { ArrowLeft, ArrowRight, BookOpenText, FileCheck2, GraduationCap, Languages, Menu, Search, Send, ShieldCheck } from "lucide-react";
...
export function SiteShell({ lang, title, eyebrow, children }: { lang: Lang; title: string; eyebrow: string; children: ReactNode }) {
  ...
  const links = [
    ...
    { to: "/auth", label: en ? "Teacher sign-in" : "دخول المدرّسين", icon: GraduationCap },
  ];
  ...
  return <div className="brand-surface min-h-screen bg-background text-foreground" dir={en ? "ltr" : "rtl"}>
    <header className="sticky top-0 z-50 border-b border-brand-gold/30 bg-brand-navy text-primary-foreground shadow-sm">
      <div className="mx-auto flex h-[72px] max-w-7xl items-center justify-between gap-4 px-5">
        <Link to={base} className="flex min-w-0 items-center gap-3" title={`${brand[lang].short} — ${brand[lang].full}`}><span className="relative grid size-11 shrink-0 place-items-center rounded-md border border-brand-gold text-brand-gold"><span className="font-display-ar text-xl font-bold">مـ</span><span className="absolute inset-1 rounded-sm border border-brand-gold/25" /></span><span className="min-w-0"><b className="block truncate font-display-ar text-xl leading-none">{brand[lang].short}</b><small className="mt-1 block text-[10px] text-brand-gold-soft">SSEBA</small></span></Link>
    ...
    <footer className="mt-16 border-t border-brand-gold/25 bg-brand-navy text-primary-foreground"><div className="mx-auto grid max-w-7xl gap-6 px-5 py-9 md:grid-cols-[1fr_auto] md:items-center"><div><div className="flex items-center gap-2 font-display-ar text-lg text-brand-gold-soft"><ShieldCheck className="size-5" />{brand[lang].short}</div><p className="mt-2 max-w-2xl text-xs leading-6 text-primary-foreground/65">{copy[lang].advisory}</p></div><Link to={base} className="flex items-center gap-2 text-sm text-brand-gold-soft">{en ? <ArrowLeft className="size-4" /> : <ArrowRight className="size-4" />}{t.back}</Link></div></footer>
  </div>;
}
