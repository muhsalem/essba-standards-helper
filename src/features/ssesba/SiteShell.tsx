...
export function SiteShell({ lang, title, eyebrow, children }: { lang: Lang; title: string; eyebrow: string; children: ReactNode }) {
  ...
  return <div className="brand-surface min-h-screen bg-background text-foreground" dir={en ? "ltr" : "rtl"}>
    ...
    <footer className="mt-16 border-t border-brand-gold/25 bg-brand-navy text-primary-foreground"><div className="mx-auto grid max-w-7xl gap-6 px-5 py-9 md:grid-cols-[1fr_auto] md:items-center"><div><div className="flex items-center gap-2 font-display-ar text-lg text-brand-gold-soft"><ShieldCheck className="size-5" />{brand[lang].short} · {brand[lang].acronym}</div><p className="mt-2 max-w-2xl text-xs leading-6 text-primary-foreground/65">{copy[lang].advisory}</p></div><Link to={base} className="flex items-center gap-2 text-sm text-brand-gold-soft">{en ? <ArrowLeft className="size-4" /> : <ArrowRight className="size-4" />}{t.back}</Link></div></footer>
  </div>;
}
