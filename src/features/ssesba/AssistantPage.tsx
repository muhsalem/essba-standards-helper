...
const labels = {
  ar: {
    ...
    desc: "الصق نصًا من المعايير أو اطرح سؤالًا عنها، فيُنتج المساعد شرحًا مبسطًا وإجابة مرتبطة بمحتوى معايير التصنيف الشرعي (م.ش.ت.ق.أ · SSESBA) دون الخروج عنه.",
...
export function AssistantPage({ lang }: { lang: Lang }) {
  ...
  async function onSubmit() {
    ...
    try {
      const result = await ask({ data: { question: question.trim(), excerpt: excerpt.trim() || undefined, lang } });
      setAnswer(result);
    } catch (err) {
        ...
        <div className="rounded-xl border border-brand-gold/25 bg-card p-6 shadow-sm">
          ...
          <div className="grid gap-5">
            <div className="grid gap-2">
              ...
              <Textarea id="assistant-question" rows={4} value={question} onChange={(event) => setQuestion(event.target.value)} placeholder={t.questionPlaceholder} />
            </div>
            <div className="grid gap-2">
          ...
          {answer ? (
            <>
              <article className="rounded-xl border border-brand-gold/25 bg-card p-6 shadow-sm">
                <h2 className="mb-3 flex items-center gap-2 font-display-ar text-lg text-brand-navy"><MessagesSquare className="size-5 text-brand-gold" />{t.summaryTitle}</h2>
                <p className="whitespace-pre-line text-sm leading-7 text-foreground/85">{answer.summary}</p>
              </article>
                ...
                <article className="rounded-xl border border-brand-gold/25 bg-card p-6 shadow-sm">
                  ...
                  <ul className="grid gap-2 text-sm leading-7 text-foreground/85">
                    {answer.points.map((point) => (
                      <li key={point} className="flex gap-2"><span className="mt-2 size-1.5 shrink-0 rounded-full bg-brand-gold" />{point}</li>
                    ))}
                  </ul>
                </article>
              ) : null}
