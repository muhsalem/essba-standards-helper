...
import { AlertTriangle, Building2, CheckCircle2, Landmark, Save, ShieldAlert, Sparkles } from "lucide-react";
...
import {
  axes, brand, calculateAssessment, complianceLevels, copy, financialScreens, gateChecks, methodologyVersion, riskTiers, specialStates,
  structuralFailureThreshold, type AssessmentMode, type FinancialFigures, type GateState, type Lang, type RiskTier, type SpecialStateId,
} from "@/lib/ssesba-data";
import { getAssessmentExamples, saveAssessmentResult, suggestAxisScores } from "@/lib/ssesba.functions";
...
const labels = {
  ar: {
    ...
    referred: "محال إلى الهيئة (قيد الدراسة)", not_applicable: "غير منطبق / خارج النطاق",
    screens: "الفرز المالي الكمي (اختياري)", screensNote: "حدود على نهج معيار أيوفي الشرعي رقم 21. تجاوز أي حد يُسقط الأهلية كبقية اختبارات البوابة. لا يُقيَّم الحد إذا لم يُدخل مقامه.",
    totalAssets: "إجمالي الأصول", interestBearingDebt: "الديون الربوية", interestBearingDeposits: "الودائع والاستثمارات الربوية", totalRevenue: "إجمالي الإيرادات", nonCompliantRevenue: "الدخل غير المباح",
    screenFailed: "تجاوز حد الفرز المالي:", special: "حالة خاصة", specialNone: "لا توجد — تقييم بالدرجات",
    save: "حفظ النتيجة برقم مرجعي", saving: "جارٍ الحفظ…", saved: "رقم النتيجة المرجعي", savedNote: "احتفظ بهذا الرقم للمراجعة أو الاعتراض. نسخة المنهجية:",
  ...
  en: {
    ...
    referred: "Referred to the board (under study)", not_applicable: "Not applicable / out of scope",
    screens: "Quantitative financial screens (optional)", screensNote: "Limits follow AAOIFI Shariah Standard No. 21. Exceeding any limit removes eligibility like the other gate tests. A screen is not evaluated when its denominator is empty.",
    totalAssets: "Total assets", interestBearingDebt: "Interest-bearing debt", interestBearingDeposits: "Interest-bearing deposits and investments", totalRevenue: "Total revenue", nonCompliantRevenue: "Non-permissible income",
    screenFailed: "Financial screen exceeded:", special: "Special state", specialNone: "None — score-based assessment",
    save: "Save result with a reference", saving: "Saving…", saved: "Result reference", savedNote: "Keep this number for review or objection. Methodology version:",
...
export function AssessmentPage({ lang }: { lang: Lang }) {
  ...
  const [scores, setScores] = useState<Record<string, number>>(Object.fromEntries(axes.map((a) => [a.id, a.sampleScore])));
  ...
  const result = useMemo(() => calculateAssessment(scores, gate, risk, { figures, specialState }), [scores, gate, risk, figures, specialState]);
  // أي تعديل في المدخلات يُبطل الرقم المرجعي السابق حتى لا يُنسب إلى نتيجة مختلفة.
  useEffect(() => { setSavedRef(""); }, [scores, gate, risk, figures, specialState]);
  ...
  async function runAI() {
    ...
    try {
      const r = await suggest({ data: { description, lang, sector: current ? (lang === "ar" ? current.title_ar : current.title_en) : undefined } });
  ...
  async function saveResult() {
    ...
    try {
      const r = await save({ data: { scores: scores as Record<(typeof axes)[number]["id"], number>, gate: gate as Record<(typeof gateChecks)[number]["id"], boolean>, risk, specialState, figures, mode, exampleId: exampleId || undefined } });
      ...
      <div className="mx-auto max-w-7xl px-5 py-10">
        ...
        <div className="grid gap-8 lg:grid-cols-[1.25fr_.75fr]">
          <section className="space-y-7">
            ...
            <div className="rounded-md border bg-card p-5">
              <h2 className="flex items-center gap-2 text-base font-semibold"><Landmark className="size-5 text-brand-gold" />{t.screens}</h2>
              <p className="mt-2 text-xs leading-5 text-muted-foreground">{t.screensNote}</p>
              <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {(["totalAssets", "interestBearingDebt", "interestBearingDeposits", "totalRevenue", "nonCompliantRevenue"] as const).map((key) => (
                  <div key={key}>
                    <Label htmlFor={`fig-${key}`}>{t[key]}</Label>
                    <Input id={`fig-${key}`} type="number" inputMode="decimal" min={0} step="any" className="mt-2" value={figures[key] ?? ""} onChange={(e) => setFigure(key, e.target.value)} />
                  </div>
                ))}
              </div>
              <ul className="mt-4 grid gap-2 text-sm">
                {result.screens.map((item) => (
                  <li key={item.screen.id} className={`flex items-center justify-between gap-3 rounded-md border px-3 py-2 ${item.passed === false ? "border-destructive/50 bg-destructive/10 text-destructive" : ""}`}>
                    <span>{item.screen[lang]} (≤ {item.screen.max}%)</span>
                    <span className="font-mono">{item.ratio === null ? "—" : `${item.ratio}%`}</span>
                  </li>
                ))}
              </ul>
              {result.failedScreens.length > 0 && (
                <p role="alert" className="mt-3 text-sm font-semibold text-destructive">{t.screenFailed} {result.failedScreens.map((item) => item.screen[lang]).join(lang === "ar" ? "، " : ", ")}</p>
              )}
            </div>
            ...
            <div aria-live="polite" aria-atomic="true" className="overflow-hidden rounded-md border bg-card shadow-sm">
              ...
              <div className="space-y-5 p-6">
                ...
                <div>
                  ...
                  <Select value={specialState ?? "none"} onValueChange={(v) => setSpecialState(v === "none" ? null : (v as SpecialStateId))}>
                    <SelectTrigger className="mt-2" aria-describedby="special-guidance"><SelectValue /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="none">{t.specialNone}</SelectItem>
                      {specialStates.map((state) => <SelectItem key={state.id} value={state.id}>{state[lang]}</SelectItem>)}
                    </SelectContent>
                  </Select>
                  {result.special && <p id="special-guidance" className="mt-2 text-xs text-muted-foreground">{result.special[lang === "ar" ? "arHelp" : "enHelp"]}</p>}
                </div>
                ...
                <div className="border-t pt-4">
                  <Button type="button" onClick={saveResult} disabled={saving} variant="outline" className="w-full"><Save />{saving ? t.saving : t.save}</Button>
                  {savedRef && (
                    <div role="status" className="mt-3 rounded-md border border-brand-gold/40 bg-brand-parchment p-3 text-sm">
                      <p className="text-xs text-muted-foreground">{t.saved}</p>
                      <p className="font-mono text-lg font-bold text-brand-navy">{savedRef}</p>
                      <p className="mt-1 text-xs text-muted-foreground">{t.savedNote} {methodologyVersion}</p>
                    </div>
                  )}
                  {error && mode !== "ai_review" && <p role="alert" className="mt-3 text-sm text-destructive">{error}</p>}
                </div>
